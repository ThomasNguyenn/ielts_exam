...
export default function DuolingoStyleHomework() {
  const lessons = [
    ...
    },
    {
      ...
      hearts: 3,
      status: 'current',
      subtitle: 'Review your answers and explanations',
      bubble: 'Today’s goal',
      color: 'from-sky-400 to-blue-500',
      offset: 'self-start ml-6 md:ml-16',
      kind: 'lesson',
    },
    {
      id: 4,
      title: 'Writing Mini Practice',
      type: 'Writing',
      icon: '✍️',
      xp: 20,
      hearts: 3,
      status: 'locked',
      subtitle: 'Unlock this after finishing today’s lesson',
      bubble: 'Coming next',
      color: 'from-violet-400 to-fuchsia-500',
      offset: 'self-end mr-3 md:mr-10',
      kind: 'lesson',
    },
  ];
                ...
                if (lesson.kind === 'chest') {
                  return (
                    <motion.div
                      ...
                      className={`relative z-10 flex flex-col items-center ${lesson.offset}`}
                    >
                      <div className="mb-3 rounded-full bg-yellow-50 px-4 py-2 text-xs font-black uppercase tracking-wide text-yellow-700 shadow-sm ring-1 ring-yellow-200">
                        {lesson.bubble}
                      </div>

                      <button className="relative flex h-24 w-24 items-center justify-center rounded-[30px] bg-gradient-to-br from-yellow-400 to-amber-500 text-4xl shadow-[0_12px_0_0_rgba(0,0,0,0.15)] transition hover:scale-110">
                        {lesson.icon}
                        <span className="absolute -top-2 -right-2 rounded-full bg-white px-2 py-1 text-xs font-bold shadow">
                          {lesson.reward}
                        </span>
                      </button>
                    </motion.div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-[30px] border border-white/80 bg-white/90 p-5 shadow-[0_14px_35px_rgba(15,23,42,0.07)]">
              ...
              <div className="space-y-3 text-sm text-slate-600">
                ...
                <div className="rounded-[22px] bg-slate-50 p-4">
                  <p className="font-black text-slate-800">Reward chest node</p>
                  <p className="mt-1 leading-6">Adds a satisfying milestone between lessons and makes the journey feel more game-like.</p>
                </div>
                <div className="rounded-[22px] bg-slate-50 p-4">
