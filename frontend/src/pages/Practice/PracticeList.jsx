...
export default function PracticeList() {
    ...
    return (
        <div className="page practice-list-page" style={{ maxWidth: '80vw', width: '100%', margin: '0 auto', padding: '2rem' }}>
            <div className="practice-header" style={{ marginBottom: '2rem' }}>
                <h1 style={{ fontSize: '2rem', color: '#1e3a8a', marginBottom: '1rem' }}>Thư viện luyện viết Writing</h1>
            ...
            <div className="practice-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
                {filteredWritings.map(w => (
                    <div key={w._id} className="practice-card" style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1rem' }}>
                            <span className={`badge ${w.task_type === 'task1' ? 'badge-blue' : 'badge-purple'}`}
                                style={{ background: w.task_type === 'task1' ? '#e0f2fe' : '#f3e8ff', color: w.task_type === 'task1' ? '#0369a1' : '#7e22ce', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 600 }}>
                                {w.task_type === 'task1' ? 'Task 1' : 'Task 2'}
                            </span>
                        </div>
                        <h3 style={{ fontSize: '1.25rem', marginBottom: '0.5rem', color: '#1e293b', flex: 1 }}>{w.title}</h3>
                        ...
                        {w.task_type === 'task1' || w.task_type === 1 ? (
                            <button className="btn btn-secondary" disabled style={{ textAlign: 'center', opacity: 0.6, cursor: 'not-allowed' }}>
                                Đang bảo trì
                            </button>
                        ) : (
                            <Link to={`/practice/${w._id}`} className="btn btn-primary" style={{ textAlign: 'center' }}>
                                Bắt đầu luyện tập
                            </Link>
                        )}
                    </div>
